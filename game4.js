...
function rollDice() {
  ...
  if (num % 2 === 0) {
    ...
    messageField.innerHTML =
      "Player&nbsp;<span class='spanXO'>X</span>&nbsp;starts";
  } else {
    playerX = false;
    messageField.innerHTML =
      "Player&nbsp;<span class='spanXO'>O</span>&nbsp;starts";
  }
}
...
function game() {
  ...
  if (this.innerText) {
    if (playerX)
      messageField.innerHTML =
        "Spot taken. Player:&nbsp;<span class='spanXO'>X</span>";
    else
      messageField.innerHTML =
        "Spot taken. Player:&nbsp;<span class='spanXO'>O</span>";
  } else {
    //look for winner
    const checkForWinner = (move) => {
      ...
      if (isWinner) {
        ...
        displayWinning(move);
      } else if (drawCount === 9) displayDraw();
    };
    //player X moves
    if (playerX) {
...
function displayWinning(winner) {
  setTimeout(() => {
    ...
    if (winner === "X") winnerName.innerText = playerX.innerText;
    else winnerName.innerText = playerO.innerText;
...
function scoreTracker(move) {
  ...
  if (move === "X") playerX.innerText++;
  else playerO.innerText++;
}
